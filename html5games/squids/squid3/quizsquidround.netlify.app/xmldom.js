...
(function() {
    ...
    var INVALID_ACCESS_ERR = ExceptionCode.INVALID_ACCESS_ERR = (ExceptionMessage[15] =
        "Invalid access", 15);
    ...
    function DOMImplementation(features) {
        this._features = {};
        if (features)
            for (var feature in features) this._features =
                features[feature]
    }
        ...
        normalize: function() {
            ...
            while (child) {
                ...
                if (next && next.nodeType ==
                    TEXT_NODE && child.nodeType == TEXT_NODE) {
    ...
    function _visitNode(node, callback) {
        ...
        if (node =
            node.firstChild) {
            do
                if (_visitNode(node, callback)) return true; while (node = node.nextSibling)
        }
    }
    ...
    function _onAddAttribute(doc, el, newAttr) {
        ...
        if (ns == "http://www.w3.org/2000/xmlns/") el._nsMap[newAttr.prefix ? newAttr.localName : ""] = newAttr.value
    }

    function _onRemoveAttribute(doc, el, newAttr, remove) {
        doc && doc._inc++;
        var ns = newAttr.namespaceURI;
        if (ns == "http://www.w3.org/2000/xmlns/") delete el._nsMap[newAttr.prefix ? newAttr.localName : ""]
    }
    ...
    function _insertBefore(parentNode, newChild, nextChild) {
        ...
        if (nextChild == null) parentNode.lastChild =
            newLast;
    ...
    function nodeSerializeToString(isHtml, nodeFilter) {
        ...
        serializeToString(this,
            buf, isHtml, nodeFilter, visibleNamespaces);
        return buf.join("")
    }
    ...
    function serializeToString(node, buf, isHTML, nodeFilter,
        ...
        switch (node.nodeType) {
            ...
            case CDATA_SECTION_NODE:
                return buf.push("<![CDATA[", node.data, "]]\x3e");
            case COMMENT_NODE:
                ...
                switch (source.charAt(tagStart + 1)) {
                    case "/":
                        ...
                        if (endIgnoreCaseMach) {
                            ...
                            if (localNSMap)
                                for (var prefix in localNSMap) domBuilder.endPrefixMapping(prefix);
                            if (!endMatch) errorHandler.fatalError("end tag name: " +
                ...
                case '"':
                    if (s === S_EQ || s === S_ATTR) {
                        ...
                        if (p > 0) {
                            ...
                            s = S_ATTR_END
                        } else throw new Error("attribute value no end '" +
                            c + "' match");
    ...
    function appendElement(el, domBuilder, currentNSMap) {
        ...
        while (i--) {
            ...
            if (nsp > 0) {
                ...
                localName = qName;
                prefix = null;
                if (qName === "xmlns") nsPrefix = "";
                else nsPrefix = "__special_false_value__"
            }
            a.localName = localName;
            if (nsPrefix !== "__special_false_value__") {
                if (localNSMap == null) {
                    localNSMap = {};
                ...
                domBuilder.startPrefixMapping(nsPrefix,
                    value)
            }
        }
        var i = el.length;
        ...
        if (el.closed) {
            ...
            if (localNSMap)
                for (prefix in localNSMap) domBuilder.endPrefixMapping(prefix)
        } else {
    ...
    function parseHtmlSpecialContent(source, elStartEnd, tagName, entityReplacer, domBuilder) {
        if (/^(?:script|textarea)$/i.test(tagName)) {
            ...
            if (/[&<]/.test(text)) {
                ...
                text =
                    text.replace(/&#?\w+;/g, entityReplacer);
                domBuilder.characters(text, 0, text.length);
                return elEndStart
            }
        }
        ...
        switch (next) {
            ...
            default:
                if (source.substr(start + 3, 6) == "CDATA[") {
                    var end = source.indexOf("]]\x3e", start + 9);
...
(function() {
    ...
    DOMParser.prototype.parseFromString = function(source, mimeType) {
        ...
        var entityMap = isHTML ? defaultEntityMap : {
            "lt": "<",
            "gt": ">",
            "amp": "&",
            "quot": '"',
        ...
        if (isHTML) defaultNSMap[""] = "http://www.w3.org/1999/xhtml";
    ...
    DOMHandler.prototype = {
        ...
        startElement: function(namespaceURI, localName, qName, attrs) {
            ...
            this.locator && position(this.locator,
                el);
            for (var i = 0; i < len; i++) {
